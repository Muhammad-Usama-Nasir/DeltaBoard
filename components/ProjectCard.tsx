...
export default function ProjectCard({
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
            ...
            {/* Header */}
            <div
                className="flex flex-col sm:flex-row sm:items-center justify-between px-4 sm:px-5 py-4 cursor-pointer hover:bg-gray-50 transition-colors gap-2 sm:gap-0"
                onClick={() => setOpen(!open)}
            >
                {/* Left side — project info */}
                <div className="flex flex-wrap items-center gap-2">
                    <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded font-mono">{project.id.slice(0, 10)}...</span>
            ...
            {/* Changes list */}
            {open && (
                <div className="border-t border-gray-200 px-4 sm:px-5">

                    {/* Column headers — hidden on mobile */}
                    <div className="hidden sm:grid sm:grid-cols-[1fr_100px_140px_32px] gap-3 py-2 border-b border-gray-200">
                    ...
                    {showForm ? (
                        ...
                        <button
                            ...
                            className="text-xs font-medium text-white bg-gray-900 hover:bg-gray-700 px-3 py-1.5 rounded-lg transition-colors my-3 flex items-center gap-1"
                        >
                            <span className="text-base leading-none">+</span> Add change
                        </button>
                    )}
                </div>
            )}
